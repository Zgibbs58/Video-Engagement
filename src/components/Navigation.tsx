        ...
        <div className="flex items-center justify-between py-2 sm:px-8 sm:px-12">
          ...
          >
            ...
            <motion.div className={`absolute top-0 right-0 bg-gray-500 text-white w-full h-full rounded-md text-5xl ${isOpen ? 'flex flex-col justify-evenly' : 'hidden'}`}>
              <NavLink linkName={"Home"} linkPath={"/"} onClick={closeMenu}/>
              <NavLink linkName={"About"} linkPath={"/about"} onClick={closeMenu}/>
              <NavLink linkName={"Portfolio"} linkPath={"/projects"} onClick={closeMenu}/>
              <NavLink linkName={"Contact"} linkPath={"/contact"} onClick={closeMenu}/>
            </motion.div>
          </motion.nav>
